...
function renderRepeatPurchases(data) {
    ...
    const totalPurchases = counts.reduce((sum, value) => sum + value, 0);

    const percentages = counts.map(value => ((value / totalPurchases) * 100).toFixed(1));
...
function renderRevenue(data) {
    ...
    data.forEach(sale => {
        ...
        grouped[month] = (grouped[month] || 0) + sale.amount;
    });

    const labels = Object.keys(grouped).sort();
    const values = labels.map(m => grouped[m]);
